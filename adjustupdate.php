...
function order_delete($OrdNo, $cId) {
  global $conn_hannam;
  $delOrder_query = "DELETE FROM trAdjustDetail ".
            "WHERE tAdjNo = '$OrdNo' AND CID = '$cId' ";
  ...
  $delOrder_query = "DELETE FROM trAdjustMaster ".
            "WHERE tAdjNo = '$OrdNo' AND CID = '$cId' ";
...
function order_confirm($OrdNo, $cId){
  global $conn_hannam;
  $conOrder_query = "UPDATE trAdjustMaster SET tStatus = '4' ".
            "WHERE tAdjNo = '$OrdNo' AND CID = '$cId' ";
...
function order_update($OrdNo, $cId){
  global $conn_hannam;
  $upOrder_query = "UPDATE trAdjustMaster SET tStatus = '2' ".
            "WHERE tAdjNo = '$OrdNo' AND CID = '$cId' ";
...
if($mode == "update") {
    ...
    echo("<script>
			...
			location.replace('?page=frame&menu=confirmed');
		</script>");
		return;
	}
}
...
if($mode == "confirm") {
    ...
    echo("<script>
			...
			location.replace('?page=frame&menu=adjust&order_no=$tOrdNo');
		</script>");
		return;
	}
}
...
if($mode == "delete" || $mode == "modify") {
	...
	if($mode == "delete")
	{
		echo("<script>location.replace('?page=frame&menu=confirmed');</script>");
		return;
	}
}
$tOrdNo = "";
...
for($i = 1; $i <= $item_num; $i++) {
  ...
  $tMemo = addslashes($tMemo);
    ...
    //		commit();
    		...
    		$addOrder_query = "INSERT INTO trAdjustDetail (CID,tAdjNo,tID,tDate,tProd,tQty,ProdEname,ProdKname,tGalcode,tProdOwnCode,tSize,tPunit,Memo) ".
    						  "VALUES ('$CID','$tOrdNo',$i,'$tDate','$tProd',".$tQty.",'".$ProdEname."', '".Br_dconv($ProdKname)."','$tGalCode','$tProdOwnCode','$tSize','$tUnit','".Br_dconv($tMemo)."') ";
	...
	if($mode == "modify") {
		...
		$modOrder_query = "INSERT INTO trAdjustDetail (CID,tAdjNo,tID,tDate,tProd,tQty,ProdEname,ProdKname,tGalcode,tProdOwnCode,tSize,tPunit,Memo) ".
              "VALUES ('$CID','$tOrdNo',$i,'$tDate','$tProd',".$tQty.",'".$ProdEname."', '".Br_dconv($ProdKname)."','$tGalCode','$tProdOwnCode','$tSize','$tUnit','".Br_dconv($tMemo)."') ";
...
if ($Insert_Record) {
  ...
  $main_memo = Br_dconv($main_memo);
	$master_query = "INSERT INTO trAdjustMaster (CID,tAdjNo,tDate,tMemo,tStatus) ".
				  "VALUES ('$CID', '$tOrdNo', '$tDate', '$main_memo', '$tStatus') ";
  // echo $query;
